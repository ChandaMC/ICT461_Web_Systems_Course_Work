...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST["action"])) {
        ...
        elseif ($_POST["action"] == "register") {
            ...
            $stmt = $conn->prepare("INSERT INTO users (Username, FullName, ContactNumber, Email, Password, VerificationToken) VALUES (?, ?, ?, ?, ?, ?)");
            $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
            $stmt->bind_param("ssssss", $username, $fullName, $contactNumber, $email, $hashedPassword, $verificationToken);

            if ($stmt->execute()) {
                // Send verification email
                $activationLink = "http://example.com/activate.php?token=" . $verificationToken;
                sendmail($email, "Account Activation", 'Click the following link to activate your account: <a href="' . $activationLink . '">Activate Now</a>');
